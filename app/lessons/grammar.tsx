...
export default function GrammarLesson() {
      ...
      try {
        ...
        if (recording) {
          ...
          if (uri) {
            ...
            const transcribedText = await sendVoiceForTranscription(uri);
            
            if (transcribedText) {
              // Set the transcribed text as user input
              setUserInput(transcribedText);
              
              // Process as if user had typed and sent the message
              ...
              const newHistory = buildHistoryString([
                ...conversation,
                { speaker: 'user', message: transcribedText }
              ]);
              
              try {
                const teacherMessage = await fetchTeacherResponse(transcribedText, newHistory);
                ...
                await playTextWithElevenLabsTTS(teacherMessage, setIsLoading);
              } catch (err) {
  ...
  // Function to send voice recording for transcription
  async function sendVoiceForTranscription(audioUri: string) {
    try {
      ...
      formData.append('file', {
        uri: audioUri,
        type: 'audio/wav',  // Adjust based on your recording format
        name: 'recording.wav',
      } as any);
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to transcribe voice: ${response.status} ${errorText}`);
      }
      ...
      Alert.alert('Error', err.message || 'Failed to transcribe voice');
      return null;
    }
  }
